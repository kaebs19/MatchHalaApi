...
class NotificationService {
    ...
    async sendFirebasePush(deviceToken, title, body, data = {}) {
        ...
        try {
            ...
            // لو التوكن منتهي، احذفه (كلا الحقلين)
            if (error.code === 'messaging/registration-token-not-registered' ||
                error.code === 'messaging/invalid-registration-token') {
                const User = require('../models/User');
                await User.updateMany(
                    { $or: [{ deviceToken }, { fcmToken: deviceToken }] },
    ...
    async sendAPNsNotification(deviceToken, notification) {
        try {
            ...
            if (result.failed.length > 0) {
                ...
                // حذف التوكن الفاسد من قاعدة البيانات
                if (failedDevice.response?.reason === 'BadDeviceToken') {
                    try {
                        ...
                        console.log('🗑️ تم حذف Device Token فاسد (APNs BadDeviceToken)');
                    } catch (cleanupErr) {
