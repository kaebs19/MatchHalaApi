                ...
                <div className="modal-overlay" onClick={() => setShowQuickActions(false)}>
                    <div className="quick-actions-panel" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="qap-actions">
                            ...
                            {/* ✅ تصعيد تلقائي */}
                            <button className="qap-btn escalate" style={{background:"#2196F3",color:"#fff"}} onClick={async () => {
                                try {
                                    const res = await fetch('/api/users/' + quickActionUser._id + '/escalate', { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + localStorage.getItem("token") }, body: JSON.stringify({ reason: "تصعيد من لوحة التحكم" }) });
                                    const data = await res.json();
                                    if (data.success) { toast.success(data.message); fetchUsers(); setShowQuickActions(false); }
                                    else toast.error(data.message);
                                } catch(e) { toast.error("فشل التصعيد"); }
                            }} disabled={quickActionLoading}>🔄 تصعيد تلقائي</button>

                            {/* ✅ تقييد جزئي */}
                            <button className="qap-btn" style={{background:"#FF9800",color:"#fff"}} onClick={async () => {
                                try {
                                    const res = await suspendUser(quickActionUser._id, "restrict-new", "تقييد جزئي من الأدمن");
                                    if (res.success) { toast.success(res.message || "تم التقييد الجزئي"); fetchUsers(); setShowQuickActions(false); }
                                } catch(e) { toast.error(e.response?.data?.message || "فشل التقييد"); }
                            }} disabled={quickActionLoading}>🔒 تقييد جزئي 24س</button>

                            {/* ✅ تقييد كامل */}
                            <button className="qap-btn" style={{background:"#f44336",color:"#fff"}} onClick={async () => {
                                try {
                                    const res = await suspendUser(quickActionUser._id, "restrict-all", "تقييد كامل من الأدمن");
                                    if (res.success) { toast.success(res.message || "تم التقييد الكامل"); fetchUsers(); setShowQuickActions(false); }
                                } catch(e) { toast.error(e.response?.data?.message || "فشل التقييد"); }
                            }} disabled={quickActionLoading}>⛔ تقييد كامل 48س</button>

                            {/* ✅ حظر الجهاز */}
                            <button className="qap-btn" style={{background:"#9C27B0",color:"#fff"}} onClick={async () => {
                                if (!window.confirm("هل أنت متأكد من حظر جهاز " + quickActionUser.name + "؟ هذا إجراء نهائي!")) return;
                                try {
                                    const res = await fetch('/api/users/' + quickActionUser._id + '/ban-device', { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + localStorage.getItem("token") }, body: JSON.stringify({ reason: "admin", details: "حظر جهاز من لوحة التحكم" }) });
                                    const data = await res.json();
                                    if (data.success) { toast.success(data.message); fetchUsers(); setShowQuickActions(false); }
                                    else toast.error(data.message);
                                } catch(e) { toast.error("فشل حظر الجهاز"); }
                            }} disabled={quickActionLoading}>📵 حظر الجهاز</button>
