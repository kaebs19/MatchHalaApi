        ...
        <div className="dashboard-content">
            ...
            {loading ? (
                ...
                <>
                    ...
                    {isAdmin && (
                        <div className="stats-section">
                            <h3 className="section-title">⚡ مباشر الآن</h3>
                            <div className="stats-grid">
                                <StatCard icon="🟢" value={liveStats.onlineNow} label="متصلون الآن" color="green" onClick={() => onPageChange && onPageChange('users')} />
                                <StatCard icon="📵" value={liveStats.bannedDevices.total} label="أجهزة محظورة" color="red" onClick={() => onPageChange && onPageChange('banned-devices')} />
                                <StatCard icon="📩" value={liveStats.appeals.pending} label="استئنافات معلقة" color="orange" onClick={() => onPageChange && onPageChange('appeals')} />
                                <StatCard icon="🚨" value={moderationStats.suspendedNow} label="مستخدمون موقوفون" color="deep-orange" onClick={() => onPageChange && onPageChange('users')} />
                            </div>
                        </div>
                    )}
