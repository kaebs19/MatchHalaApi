...
router.get('/blocked', auth, async (req, res) => {
    try {
        const user = await User.findById(req.user.id)
            .populate('blockedUsers', 'name email profileImage isOnline');
...
router.post('/block/:userId', [
    ...
    try {
        ...
        // إضافة للقائمة المحظورة (ثنائي الاتجاه)
        await User.findByIdAndUpdate(req.user.id, {
...
// ✅ Premium: invisibleRead toggle
router.put('/invisible-read', auth, async (req, res) => {
    try {
        const { enabled } = req.body;
        const user = await User.findById(req.user._id);
        if (!user || !user.isPremium) {
            return res.status(403).json({ success: false, message: 'هذه الميزة للمشتركين فقط', premiumRequired: true });
        }
        if (!user.privacySettings) user.privacySettings = {};
        user.privacySettings.invisibleRead = !!enabled;
        await user.save();
        res.json({ success: true, data: { invisibleRead: !!enabled } });
    } catch (error) {
        res.status(500).json({ success: false, message: 'خطأ في الخادم' });
    }
});

// ✅ Premium: custom name color
router.put('/name-color', auth, async (req, res) => {
    try {
        const { color } = req.body; // hex string like #FF6B9D or null
        const user = await User.findById(req.user._id);
        if (!user || !user.isPremium) {
            return res.status(403).json({ success: false, message: 'هذه الميزة للمشتركين فقط', premiumRequired: true });
        }
        if (color && !/^#[0-9A-Fa-f]{6}$/.test(color)) {
            return res.status(400).json({ success: false, message: 'صيغة اللون غير صالحة' });
        }
        user.customNameColor = color || null;
        await user.save();
        res.json({ success: true, data: { customNameColor: user.customNameColor } });
    } catch (error) {
        res.status(500).json({ success: false, message: 'خطأ في الخادم' });
    }
});
...
// ✅ Privacy: قبول/إيقاف طلبات المحادثة الجديدة
router.put('/accepting-requests', auth, async (req, res) => {
    try {
        const { enabled } = req.body;
        await User.findByIdAndUpdate(req.user._id, { acceptingRequests: !!enabled });
        res.json({ success: true, data: { acceptingRequests: !!enabled } });
    } catch (error) {
        res.status(500).json({ success: false, message: 'خطأ في الخادم' });
    }
});
...
router.put('/premium-only-requests', auth, async (req, res) => {
    try {
        ...
        user.premiumOnlyRequests = !!enabled;
        await user.save();
        res.json({ success: true, data: { premiumOnlyRequests: !!enabled } });
    } catch (error) {
        res.status(500).json({ success: false, message: 'خطأ في الخادم' });
    }
});
...
// ✅ Privacy: إخفاء العمر
router.put('/show-age', auth, async (req, res) => {
    try {
        const { show } = req.body;
        await User.findByIdAndUpdate(req.user._id, { showAge: !!show });
        res.json({ success: true, data: { showAge: !!show } });
    } catch (error) {
        res.status(500).json({ success: false, message: 'خطأ في الخادم' });
    }
});
...
// ✅ Privacy: إخفاء الدولة
router.put('/show-country', auth, async (req, res) => {
    try {
        const { show } = req.body;
        await User.findByIdAndUpdate(req.user._id, { showCountry: !!show });
        res.json({ success: true, data: { showCountry: !!show } });
    } catch (error) {
        res.status(500).json({ success: false, message: 'خطأ في الخادم' });
    }
});
