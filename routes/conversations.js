    ...
    try {
        ...
        const [conversations, count] = await Promise.all([
            Conversation.find(filter)
                .populate('participants', 'name email profileImage isPremium isOnline lastLogin verification.isVerified')
                .populate({
                    path: 'lastMessage',
        ...
        // تشغيل العدادات بالتوازي
        const [flaggedCounts, messageCounts, imageCounts] = await Promise.all([
            FlaggedMessage.aggregate([
