...
router.post('/', protect, async (req, res) => {
    try {
        ...
        if (type === 'superlike') {
            ...
            const userIsPremium = user.isPremium && (!user.premiumExpiresAt || user.premiumExpiresAt > new Date());
                const maxSuperLikes = userIsPremium ? 5 : 1;
...
router.get('/cards', protect, async (req, res) => {
    try {
        ...
        // --- فلتر الأشباح: آخر 30 يوم (index-friendly) ---
        const ghostCutoff = new Date();
        ghostCutoff.setDate(ghostCutoff.getDate() - 30);

        // ✅ فلتر محسّن:
        // - حذف $ne: 'private' (0 users في هذا العدد يستخدمها)
        // - استبدال $or بـ $gte مباشر (MongoDB يستخدم range scan)
        // - الـ $nin للـ swiped/blocked يُطبّق بعد الـ index filter
        const filter = {
            isActive: true,
        ...
        // ✅ تخزين في الـ cache
        cardsCache.set(cacheKey, responseData);
