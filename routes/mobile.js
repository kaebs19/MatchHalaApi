...
const uploadAudio = multer({
    ...
    fileFilter: (req, file, cb) => {
        const allowedTypes = /m4a|mp3|wav|aac|ogg|mpeg|mp4/;
        const extname = /m4a|mp3|wav|aac|ogg/.test(path.extname(file.originalname).toLowerCase().replace('.', ''));
        const mimetype = allowedTypes.test(file.mimetype);
        if (extname || mimetype) {
            cb(null, true);
        } else {
            cb(new Error('فقط الملفات الصوتية مسموحة (M4A, MP3, WAV, AAC, OGG)'));
        }
    }
});
...
router.get('/users/blocked', protect, async (req, res) => {
    try {
        const user = await User.findById(req.user._id)
            .populate('blockedUsers', 'name email profileImage isPremium isActive verification.isVerified');
...
// @access  Private
router.post('/conversations/request', protect, require('../middleware/spamDetection').spamCheckMiddleware, async (req, res) => {
    try {
        ...
        // ✅ Privacy: المستخدم المستهدف لا يقبل طلبات جديدة
        if (targetUser.acceptingRequests === false) {
            return res.status(403).json({
                success: false,
                message: 'هذا المستخدم لا يقبل طلبات محادثة جديدة حالياً',
                code: 'NOT_ACCEPTING_REQUESTS'
            });
        }

        // ✅ Premium-only requests
        if (targetUser.premiumOnlyRequests === true && !req.user.isPremium) {
            return res.status(403).json({
                success: false,
                message: 'هذا المستخدم يقبل طلبات من المشتركين فقط',
...
// @desc    إرسال رسالة
// @access  Private
router.post('/messages/send', protect, require('../middleware/spamDetection').spamCheckMiddleware, async (req, res) => {
    try {
...
router.post('/messages/send-image', protect, uploadMessageImage.single('image'), async (req, res) => {
    ...
    try {
        ...
        // ✅ Photo Privacy Lock — صورة مقفلة (blurred)
        if (req.body.isBlurred === 'true' || req.body.isBlurred === true) {
            messageData.isBlurred = true;
            messageData.blurredUnlockedFor = [];
        }
...
// @access  Private
router.post('/messages/send-audio', protect, uploadAudio.single('audio'), async (req, res) => {
    try {
        ...
        // ✅ تحليل waveform JSON إن وُجد
        let waveformArr = undefined;
        if (waveform) {
            try {
                const parsed = JSON.parse(waveform);
                if (Array.isArray(parsed)) {
                    waveformArr = parsed.slice(0, 100).map(v => Number(v) || 0);
                }
            } catch (e) { /* ignore */ }
        }
        ...
        const message = await Message.create({
            ...
            type: 'audio',
            mediaUrl: mediaUrl,
            content: duration ? String(duration) : '0',
            status: 'sent',
            audioDuration: duration ? Number(duration) : null,
            audioWaveform: waveformArr
        });
...
router.post('/reports', protect, async (req, res) => {
    try {
        ...
        // منع التكرار: لا يمكن الإبلاغ عن نفس الشخص إذا فيه بلاغ معلق
        ...
        if (existingReport) {
            return res.status(409).json({
                success: false,
                message: 'لديك بلاغ سابق على هذا المستخدم قيد المراجعة',
...
router.post('/messages/:messageId/react', protect, async (req, res) => {
    try {
        ...
        // ✅ تجاهل الرسائل المؤقتة (temp-XXX من iOS)
...
router.put('/conversations/:conversationId/chat-mode', protect, async (req, res) => {
    try {
        ...
        const populatedSystem = await Message.findById(systemMessage._id)
            .populate('sender', 'name email profileImage isActive');
...
router.post('/report-spam', protect, async (req, res) => {
    // ⚠️ معطّل تماماً (2026-04-07) — لا يحفظ بلاغات تلقائية ولا يستدعي escalation
    // البلاغات اليدوية فقط (من زر الإبلاغ في التطبيق) عبر /reports
...
// ═══════════════════════════════════════════════════════════════
// @route   POST /api/mobile/messages/:messageId/unblur
// @desc    طلب إظهار صورة مقفلة (Photo Privacy Lock)
// @access  Private
// ═══════════════════════════════════════════════════════════════
router.post('/messages/:messageId/unblur', protect, async (req, res) => {
    try {
        const { messageId } = req.params;
        const userId = req.user._id;
        ...
        if (!message.isBlurred) {
            return res.json({ success: true, message: 'الصورة مفتوحة بالفعل', alreadyUnlocked: true });
        }
        ...
        // إضافة المستخدم لقائمة من فك القفل
        if (!message.blurredUnlockedFor.some(u => u.toString() === userId.toString())) {
            message.blurredUnlockedFor.push(userId);
            await message.save();
        }

        // إخطار المرسل بأن المستلم فتح الصورة
        if (global.io) {
            global.io.to().emit('photo-unlocked', {
                messageId: message._id,
                conversationId: message.conversation,
                unlockedBy: userId
            });
        ...
        res.json({ success: true, message: 'تم فك قفل الصورة', mediaUrl: message.mediaUrl });
    } catch (error) {
...
// ═══════════════════════════════════════════════════════════════
// @route   POST /api/mobile/panic
// @desc    Panic Button — بلاغ طارئ + إنهاء المحادثة + حظر الطرف الآخر
// @access  Private
// ═══════════════════════════════════════════════════════════════
router.post('/panic', protect, async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(conversationId) || !mongoose.Types.ObjectId.isValid(reportedUserId)) {
            return res.status(400).json({ success: false, message: 'معرّفات غير صالحة' });
        }

        // 1. إنشاء بلاغ بأولوية عالية
        const Report = require('../models/Report');
        await Report.create({
            reportedBy: userId,
            ...
            type: 'user',
            priority: 'urgent',
            reason: reason || 'Panic Button — بلاغ طارئ من المستخدم',
            status: 'pending'
        });

        // 2. إنهاء المحادثة (تعطيلها)
        await Conversation.findByIdAndUpdate(conversationId, {
            isActive: false,
            status: 'rejected'
        });

        // 3. إضافة المستخدم لقائمة الحظر
        await User.findByIdAndUpdate(userId, {
            $addToSet: { blockedUsers: reportedUserId }
        });

        // 4. إخطار socket لإغلاق المحادثة فوراً
        if (global.io) {
            global.io.to(`user:${userId}`).emit('conversation-ended', { conversationId });
        }

        res.json({
            success: true,
            message: 'تم إرسال البلاغ الطارئ + حظر المستخدم + إنهاء المحادثة'
        });
    } catch (error) {
...
// ═══════════════════════════════════════════════════════════════
// 🎲 Icebreaker Games — لعبة 20 سؤال
// ═══════════════════════════════════════════════════════════════

const IcebreakerSession = require('../models/IcebreakerSession');
const ICEBREAKER_QUESTIONS = require('../config/icebreakerQuestions');

router.get('/icebreaker/:conversationId', protect, async (req, res) => {
    try {
        const { conversationId } = req.params;
        const userId = req.user._id;
        ...
        let session = await IcebreakerSession.findOne({ conversation: conversationId });
        if (!session) {
            session = await IcebreakerSession.create({
                conversation: conversationId,
                initiator: userId,
        ...
        const currentQ = ICEBREAKER_QUESTIONS[session.currentQuestionIndex] || null;
        ...
        res.json({
            ...
            data: {
                ...
                totalQuestions: ICEBREAKER_QUESTIONS.length,
        ...
        console.error('icebreaker get error:', error);
        res.status(500).json({ success: false, message: 'خطأ في الخادم' });
    }
});

router.post('/icebreaker/:conversationId/answer', protect, async (req, res) => {
    try {
        const { conversationId } = req.params;
        ...
        if (answer.length > 500) {
            return res.status(400).json({ success: false, message: 'الإجابة طويلة جداً' });
        }
        ...
        if (idx >= ICEBREAKER_QUESTIONS.length) {
            return res.status(400).json({ success: false, message: 'انتهت الأسئلة' });
        }

        const q = ICEBREAKER_QUESTIONS[idx];
        let entry = session.answers.find(a => a.questionIndex === idx);
        if (!entry) {
            entry = {
                questionIndex: idx,
                question: q.ar + ' / ' + q.en,
        ...
        if (!entry.userA || !entry.userA.user) {
            entry.userA = { user: userId, answer: answer.trim(), answeredAt: new Date() };
        } else if (entry.userA.user.toString() === userId.toString()) {
            entry.userA.answer = answer.trim();
            entry.userA.answeredAt = new Date();
        } else if (!entry.userB || !entry.userB.user) {
            entry.userB = { user: userId, answer: answer.trim(), answeredAt: new Date() };
        } else if (entry.userB.user.toString() === userId.toString()) {
            entry.userB.answer = answer.trim();
            entry.userB.answeredAt = new Date();
        }

        const bothAnswered = entry.userA && entry.userA.user && entry.userB && entry.userB.user;
        if (bothAnswered) {
            entry.revealedAt = new Date();
            session.currentQuestionIndex += 1;
            if (session.currentQuestionIndex >= ICEBREAKER_QUESTIONS.length) {
        ...
        if (global.io && bothAnswered) {
            for (const participant of conversation.participants) {
                global.io.to(`user:${participant}`).emit('icebreaker-revealed', {
                    conversationId,
                    questionIndex: idx
                });
            }
        ...
        const nextQ = ICEBREAKER_QUESTIONS[session.currentQuestionIndex] || null;
...
router.delete('/icebreaker/:conversationId', protect, async (req, res) => {
    try {
        const { conversationId } = req.params;
        if (!mongoose.Types.ObjectId.isValid(conversationId)) {
            return res.status(400).json({ success: false, message: 'معرّف غير صالح' });
        }
        await IcebreakerSession.deleteOne({ conversation: conversationId });
        res.json({ success: true, message: 'تم إنهاء اللعبة' });
    } catch (error) {
        res.status(500).json({ success: false, message: 'خطأ في الخادم' });
    }
});
...
// ═══════════════════════════════════════════════════════════════
// 🌟 Best Friends — أفضل الأصدقاء (Premium feature)
// نظام احتساب مشابه لـ Snapchat بناء على:
// - عدد الرسائل المتبادلة (آخر 30 يوم)
// - تكرار التواجد (آخر تواجد قريب)
// - عمر الصداقة
// ═══════════════════════════════════════════════════════════════
router.get('/best-friends', protect, async (req, res) => {
    try {
        ...
        for (const item of messageCounts) {
            // Score = total messages * mutual factor (تبادل الرسائل أعلى = score أعلى)
            const total = item.messageCount;
            const mine = item.myMessages;
            const theirs = total - mine;
            // كلما كان التبادل متوازن، كان الـ score أعلى
        ...
        const users = await User.find({ _id: { $in: userIds } })
            .select('name profileImage photos isOnline lastLogin isPremium verification.isVerified')
            .lean();
        ...
        // 5. تركيب النتيجة النهائية مع رتبة (rank) لتمييز الترتيب
