...
// ✅ حظر جهاز المستخدم
export const banDevice = async (userId, reason = "manual", details = "") => {
    const response = await api.post(/users/ + userId + /ban-device, { reason, details });
    return response.data;
};
...
// ✅ تصعيد تلقائي
export const escalateUser = async (userId, reason = "") => {
    const response = await api.post(/users/ + userId + /escalate, { reason });
    return response.data;
};
