...
// كاش في الذاكرة لتجنب query لكل connect/disconnect
// ═══════════════════════════════════════════════════════════════
const partnersCache = new Map(); // userId -> { partners: Set, expiry: timestamp }
const PARTNERS_CACHE_TTL = 5 * 60 * 1000; // 5 دقائق

async function getConversationPartners(userId) {
    const cached = partnersCache.get(userId);
...
// مسح كاش مستخدم عند تغيير محادثاته (محادثة جديدة/حذف/إلخ)
function invalidatePartnersCache(userId) {
    partnersCache.delete(userId);
}
// تصدير للاستخدام في mobile.js عند إنشاء/حذف محادثة
global.invalidatePartnersCache = invalidatePartnersCache;

// تنظيف الكاش المنتهي كل 10 دقائق
setInterval(() => {
    const now = Date.now();
    for (const [key, val] of partnersCache) {
        if (val.expiry < now) partnersCache.delete(key);
    }
}, 10 * 60 * 1000);
